...
reading in data from previous lecture 
```{r}
library(tidyverse)
library(readxl)
app_data <- read_excel("data/app_data.xlsx", sheet = 1)
...
check on all numeric variables 
```{r}
app_data |>
...
need to get rid of the NA value 
```{r}
app_data |>
  summarize(across(where(is.numeric), 
...
Statistics across groups with group_by function 
```{r}
app_data |>
...
can do this another fancier way 
```{r}
app_data |>
  group_by(Diagnosis, Sex) |>
  drop_na(Diagnosis, Sex) |>
...
to do this with missing variables 
```{r}
app_data |>
  group_by(Diagnosis, Sex) |>
  drop_na(Diagnosis, Sex) |>
...
## Numeric Variables and More Video 
...
remove really large values and use the fill aesthetic to compare groups 
```{r}
g <- ggplot(app_data |>
              drop_na(RBC_Count, Diagnosis) |>
              filter(RBC_Count < 8), 
            aes(x = RBC_Count))
#alpha is set to how much we want to see through the graph 0 = completely see through and 1 = not see through
